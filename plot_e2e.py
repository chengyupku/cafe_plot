...
gs = gridspec.GridSpec(
    4, 12, figure=fig, height_ratios=[1, 1, 1, 1], wspace=3.2, hspace=0.4
)


#  Figure 1.1
ax1_1 = fig.add_subplot(gs[0, 0:12])
ax1_1.set_title("LLAMA3-70B on H100")
ax1_1.set_ylim(0, 3.5)  # 下面的图为0到5

ax1_1.spines["top"].set_visible(True)

times_data = llama_times_data_h100
...
ax1_1.set_xticklabels(providers, fontsize=12)
ax1_1.grid(False)
ax1_1.set_xticks(x + len(norm_time_data) * bar_width / 2)
ax1_1.set_xticklabels(providers, fontsize=12)
...
#  Figure 1.2
ax1_2 = fig.add_subplot(gs[1, 0:12])
ax1_2.set_title("LLAMA3-70B on MI210")
ax1_2.set_ylim(0, 3)  # 下面的图为0到5

ax1_2.spines["top"].set_visible(True)

times_data = llama_times_data_mi210
providers = llama_providers
_1x_baseline = "TileSight"
_1x_baseline_times = dict(times_data)[_1x_baseline]
...
ax1_2.set_xticklabels(providers, fontsize=12)
ax1_2.grid(False)
ax1_2.set_xticks(x + len(norm_time_data) * bar_width / 2)
ax1_2.set_xticklabels(providers, fontsize=12)
...
#  Figure 1.1
ax2_1 = fig.add_subplot(gs[2, 0:12])
ax2_1.set_title("QWEN-72B on H100")
ax2_1.set_ylim(0, 2)  # 下面的图为0到5

ax2_1.spines["top"].set_visible(True)

times_data = qwen_times_data_h100
...
ax2_1.set_xticklabels(providers, fontsize=12)
ax2_1.grid(False)
ax2_1.set_xticks(x + len(norm_time_data) * bar_width / 2)
ax2_1.set_xticklabels(providers, fontsize=12)
...
#  Figure 1.2
ax2_2 = fig.add_subplot(gs[3, 0:12])
ax2_2.set_title("QWEN-72B on MI210")
ax2_2.set_ylim(0, 2.5)  # 下面的图为0到5

ax2_2.spines["top"].set_visible(True)

times_data = qwen_times_data_mi210
providers = qwen_providers
_1x_baseline = "TileSight"
_1x_baseline_times = dict(times_data)[_1x_baseline]
...
ax2_2.set_xticks(x + len(norm_time_data) * bar_width / 2)
ax2_2.set_xticklabels(providers, fontsize=12)
ax2_2.grid(False)
ax2_2.set_xticks(x + len(norm_time_data) * bar_width / 2)
ax2_2.set_xticklabels(providers, fontsize=12)
...
fig.legend(
    ...
    ncol=len(labels_other) // 2,       # 根据标签数量设置为水平排开
...
fig.text(
    0.04,
    0.5,
    "Normalized latency Vs. TileSight\n(lower is better)",
    fontsize=15,
    rotation=90,
    va="center",
    ha="center",
)
plt.subplots_adjust(top=0.88, bottom=0.15, right=0.75)
# plt.show()

plt.savefig(
    "./end2end_tilesight.pdf",
    bbox_inches="tight",
)